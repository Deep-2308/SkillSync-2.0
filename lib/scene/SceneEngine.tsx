"use client";

import React, { useRef, useMemo, useEffect } from "react";
import { Canvas, useFrame, useThree } from "@react-three/fiber";
import * as THREE from "three";
import gsap from "gsap";
import { sceneUniforms } from "./SceneUniforms";
import { sceneEventBus, SceneState } from "./SceneEvents";

const vertexShader = `
  varying vec2 vUv;
  void main() {
    vUv = uv;
    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
  }
`;

const fragmentShader = `
  uniform float uTime;
  uniform vec2 uPointer;
  uniform vec3 uColorEmber;
  uniform vec3 uColorViolet;
  uniform vec3 uColorCanvas;
  uniform float uStateBlend;
  uniform float uIntensity;

  varying vec2 vUv;

  // Classic Simplex 3D Noise by Ian McEwan, Ashima Arts
  vec4 permute(vec4 x){return mod(((x*34.0)+1.0)*x, 289.0);}
  vec4 taylorInvSqrt(vec4 r){return 1.79284291400159 - 0.85373472095314 * r;}
  float snoise(vec3 v){
    const vec2  C = vec2(1.0/6.0, 1.0/3.0) ;
    const vec4  D = vec4(0.0, 0.5, 1.0, 2.0);
    vec3 i  = floor(v + dot(v, C.yyy) );
    vec3 x0 = v - i + dot(i, C.xxx) ;
    vec3 g = step(x0.yzx, x0.xyz);
    vec3 l = 1.0 - g;
    vec3 i1 = min( g.xyz, l.zxy );
    vec3 i2 = max( g.xyz, l.zxy );
    vec3 x1 = x0 - i1 + 1.0 * C.xxx;
    vec3 x2 = x0 - i2 + 2.0 * C.xxx;
    vec3 x3 = x0 - 1.0 + 3.0 * C.xxx;
    i = mod(i, 289.0 );
    vec4 p = permute( permute( permute(
               i.z + vec4(0.0, i1.z, i2.z, 1.0 ))
             + i.y + vec4(0.0, i1.y, i2.y, 1.0 ))
             + i.x + vec4(0.0, i1.x, i2.x, 1.0 ));
    float n_ = 1.42108592; // 1.0/0.7042
    vec3  ns = n_ * D.wyz - D.xzx;
    vec4 j = p - 49.0 * floor(p * ns.z *ns.z);
    vec4 x_ = floor(j * ns.z);
    vec4 y_ = floor(j - 7.0 * x_ );
    vec4 x = x_ *ns.x + ns.yyyy;
    vec4 y = y_ *ns.x + ns.yyyy;
    vec4 h = 1.0 - abs(x) - abs(y);
    vec4 b0 = vec4( x.xy, y.xy );
    vec4 b1 = vec4( x.zw, y.zw );
    vec4 s0 = floor(b0)*2.0 + 1.0;
    vec4 s1 = floor(b1)*2.0 + 1.0;
    vec4 sh = -step(h, vec4(0.0));
    vec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy ;
    vec4 a1 = b1.xzyw + s1.xzyw*sh.zzww ;
    vec3 p0 = vec3(a0.xy,h.x);
    vec3 p1 = vec3(a0.zw,h.y);
    vec3 p2 = vec3(a1.xy,h.z);
    vec3 p3 = vec3(a1.zw,h.w);
    vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));
    p0 *= norm.x;
    p1 *= norm.y;
    p2 *= norm.z;
    p3 *= norm.w;
    vec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0);
    m = m * m;
    return 42.0 * dot( m*m, vec4( dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3) ) );
  }

  void main() {
    // Magnetic pull to pointer
    float dist = distance(vUv, uPointer);
    float pull = smoothstep(0.5, 0.0, dist) * 0.5 * (1.0 - uStateBlend * 0.8); // Pull diminishes in Problem state

    // Speed modifier based on state (slower in Problem state)
    float timeMod = uTime * mix(0.1, 0.02, uStateBlend);
    
    // Noise scale (more rigid/compressed in Problem state)
    float scale1 = mix(3.0, 8.0, uStateBlend);
    float scale2 = mix(5.0, 15.0, uStateBlend);

    // Flow field
    float n1 = snoise(vec3(vUv * scale1, timeMod));
    float n2 = snoise(vec3(vUv * scale2 - pull, timeMod * 1.5));
    
    float noise = (n1 + n2) * 0.5;
    
    // Core energy mapped between Canvas and Ember/Violet
    // Lighting compresses in Problem state
    float threshold = mix(0.8, 0.5, uStateBlend);
    vec3 color = mix(uColorCanvas, uColorEmber, smoothstep(-0.2, threshold, noise + pull * 0.5) * uIntensity);
    color = mix(color, uColorViolet, smoothstep(0.2, 1.0, noise) * 0.5 * uIntensity * (1.0 - uStateBlend * 0.5));

    // Vignette
    float vignette = distance(vUv, vec2(0.5));
    color = mix(color, uColorCanvas, smoothstep(0.3 - uStateBlend * 0.1, 1.0, vignette));

    gl_FragColor = vec4(color, 1.0);
  }
`;

function ScenePlane() {
  const materialRef = useRef<THREE.ShaderMaterial>(null);
  const { viewport } = useThree();

  const uniforms = useMemo(() => sceneUniforms, []);

  useFrame((state) => {
    if (materialRef.current) {
      materialRef.current.uniforms.uTime.value = state.clock.elapsedTime;
    }
  });

  return (
    <mesh scale={[viewport.width, viewport.height, 1]}>
      <planeGeometry args={[1, 1, 32, 32]} />
      <shaderMaterial
        ref={materialRef}
        vertexShader={vertexShader}
        fragmentShader={fragmentShader}
        uniforms={uniforms}
        depthWrite={false}
        depthTest={false}
      />
    </mesh>
  );
}

export function SceneEngine() {
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      // Normalize to 0.0 -> 1.0 based on viewport
      sceneUniforms.uPointer.value.x = e.clientX / window.innerWidth;
      // Invert Y for UV coordinates (0 at bottom, 1 at top in WebGL)
      sceneUniforms.uPointer.value.y = 1.0 - (e.clientY / window.innerHeight);
    };
    
    window.addEventListener("mousemove", handleMouseMove);
    
    // Listen to Event Bus
    const unsubscribe = sceneEventBus.subscribe((payload) => {
      let targetBlend = 0;
      if (payload.state === "problem") targetBlend = 1;
      
      gsap.to(sceneUniforms.uStateBlend, {
        value: targetBlend,
        duration: 1.5,
        ease: "power2.inOut",
        overwrite: true
      });
    });

    return () => {
      window.removeEventListener("mousemove", handleMouseMove);
      unsubscribe();
    };
  }, []);

  return (
    <div className="fixed inset-0 -z-50 pointer-events-none bg-canvas">
      <Canvas
        camera={{ position: [0, 0, 1] }}
        dpr={[1, 1.5]} // Performance: cap at 1.5
        gl={{ alpha: false, antialias: false, powerPreference: "high-performance" }}
      >
        <ScenePlane />
      </Canvas>
    </div>
  );
}
